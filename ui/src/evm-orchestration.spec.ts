...
test('toEip2098 converts 65-byte signature to 64-byte compact', ({ expect }) => {
  ...
  expect(compact.slice(2, 66)).toBe('1234567890123456789012345678901234567890123456789012345678901234');
  
  // vs should be s without high bit (since v=27)
  const vs = compact.slice(66);
  expect(vs).toBe('abcdef0123456789abcdef0123456789abcdef0123456789abcdef0123456789');
});
