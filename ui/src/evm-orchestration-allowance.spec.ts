...
test('ensurePermit2Allowance should query allowance from publicClient', async ({ expect }) => {
  ...
  try {
    await ensurePermit2Allowance(walletClient, amount, onProgress);
    
    // Should have recorded the allowance check
    expect(messages.some(msg => msg.includes('USDC allowance to Permit2'))).toBe(true);
  } catch (error) {
